                               ...
                               uint32_t *out_hwpt, int *out_fault_fd)
{
    ...
    struct iommu_hwpt_user_data user_data = {
        .config_len = len,
        .__reserved = 0,
        .config_uptr = (uint64_t)data_ptr,
        .eventfd = fd,
    };
    struct iommu_hwpt_alloc alloc_hwpt = {
        .size = sizeof(struct iommu_hwpt_alloc),
        .flags = 1 /* IOMMU_HWPT_ALLOC_ENFORCE_DIRTY */,
        .dev_id = dev_id,
        .pt_id = pt_id,
        .hwpt_type = hwpt_type,
        .data_len = sizeof(user_data),
        .data_uptr = (uint64_t)&user_data,
        .__reserved = 0,
    };
    ...
    if (ret) {
        ...
        if (out_fault_fd)
            *out_fault_fd = user_data.out_fault_fd;
                                   ...
                                   uint64_t page_size, uint64_t *data)
{
    ...
    ret = ioctl(be->fd, IOMMU_HWPT_GET_DIRTY_IOVA, &get_dirty_iova);
