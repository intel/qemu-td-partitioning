    ...
    while (true) {
        ...
        *writes = (*writes) && (slpte & VTD_SL_W);
        ...
        if (vtd_is_last_slpte(slpte, level)) {
            pe_flags = vtd_sm_pasid_entry_flags(s, ce, pasid);
                                   ...
                                   IOMMUTLBEntry *entry)
{
    ...
    if (!rid2pasid) {
        ...
        if (iotlb_entry) {
            ...
            if (vtd_ssad_update(s, iotlb_entry->sm_pe_flags, &slpte, is_write,
                ...
                offset = vtd_iova_level_offset(addr, vtd_get_iova_level(s, &ce, pasid));
    ...
    vtd_update_iotlb(s, source_id, vtd_get_domain_id(s, &ce, pasid),
                     ...
                     vtd_sm_pasid_entry_flags(s, &ce, pasid));
out:
    vtd_iommu_unlock(s);
    entry->iova = addr & page_mask;
...
static bool vtd_process_wait_desc(IntelIOMMUState *s, VTDInvDesc *inv_desc)
{
    ...
    if (inv_desc->lo & VTD_INV_DESC_WAIT_SW) {
        ...
        printf("%s this is a fence wait desc: hi: %llx, lo: %llx\n",
               __func__, (unsigned long long) inv_desc->hi,
               (unsigned long long) inv_desc->lo);
        /*
                                      ...
                                      struct iommu_page_response *pg_resp)
{
    ...
    idev = vtd_idev->idev;
    ret = iommufd_backend_page_response(hwpt->iommufd, hwpt->hwpt_id,
                                        idev->dev_id, pg_resp);
out_unlock:
    vtd_iommu_unlock(s);
    return ret;
                                  ...
                                  bool identical, uint32_t *pasid)
{
    ...
    return iommufd_backend_alloc_pasid(s->iommufd->fd, VTD_HPASID_MIN,
...
static int __vtd_free_host_pasid(IntelIOMMUState *s, uint32_t pasid)
{
    ...
    ret = iommufd_backend_free_pasid(s->iommufd->fd, pasid);
    if (ret < 0) {
