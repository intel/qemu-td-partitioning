...
static int iommufd_query_dirty_bitmap(VFIOContainer *bcontainer,
                                      ...
                                      uint64_t size)
{
    ...
    QLIST_FOREACH(hwpt, &container->hwpt_list, next) {
        ret = iommufd_backend_get_dirty_iova(container->be, hwpt->hwpt_id,
                                             iova, size, page_size,
                                             vbmap->bitmap);
        if (ret) {
            break;
...
static void
vfio_device_detach_container(VFIODevice *vbasedev,
                                        ...
                                        bool secure, Error **errp)
{
    ...
    if (ret) {
        iommufd_backend_free_id(bind.iommufd, hwpt_id);
        vfio_kvm_device_del_device(vbasedev);
...
static void vfio_iommufd_container_destroy(VFIOIOMMUFDContainer *container)
{
    ...
    QLIST_SAFE_REMOVE(&container->bcontainer, next);
    g_free(container);
}
    ...
    QLIST_FOREACH(bcontainer, &space->containers, next) {
        ...
        if (vfio_device_attach_container(vbasedev, container, secure, &err)) {
            ...
            if (ret) {
                ...
                goto err_detach_container;
            }
            goto exist_container;
        }
    }
