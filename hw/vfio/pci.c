                                   ...
                                   MSIMessage *msg, IOHandler *handler)
{
     ...
     * for the upper bound of vectors being enabled.
     */
    if ((vdev->msix->irq_info_flags & VFIO_IRQ_INFO_NORESIZE) &&
        ...
        if (vdev->nr_vectors > nr || !vdev->defer_kvm_irq_routing) {
            if (vector->virq >= 0) {
...
static void vfio_realize(PCIDevice *pdev, Error **errp)
{
    ...
    if (!vfio_check_path_with_fd(vbasedev, errp)) {
        return;
    }
...
static Property vfio_pci_dev_properties[] = {
    ...
    DEFINE_PROP_BOOL("x-enable-dynamic-mmap", VFIOPCIDevice,
                     vbasedev.enable_dynamic_mmap, false),
